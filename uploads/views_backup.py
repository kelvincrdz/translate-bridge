...
class DeleteFileView(generics.DestroyAPIView):
    ...
    def perform_destroy(self, instance):
        # Registrar informações para auditoria antes de deletar
        file_metadata = {
            'file_size': instance.file.size,
            'file_name': instance.file.name
        }
        
        # Verificar se há ExtractedEpub associado e registrar metadados
        try:
        ...
        # Deletar o arquivo (cascade irá cuidar dos relacionamentos)
        super().perform_destroy(instance)
        
        # Limpeza preventiva de registros órfãos após a deleção
        try:
...
class TranslateEpubView(generics.GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        log.info(f"[Translation] ExtractedEpub encontrado: ID={extracted.id}, title='{extracted.title}'")
        ...
        try:
            log.info(f"[Translation] Iniciando translate_epub_sync...")
            ...
            log.info(f"[Translation] Tradução concluída: translation_id={translation.id}")
            
            serializer = TranslatedEpubSerializer(translation)
            log.info(f"[Translation] Retornando dados serializados")
...
class BooksListView(generics.ListAPIView):
    ...
    """
    permission_classes = [IsAuthenticated]
    serializer_class = None  # retornaremos estrutura custom
    ...
    def list(self, request, *args, **kwargs):  # type: ignore
        ...
        data = []
        # Pré-carrega progresso para evitar N+1
...
class EpubReaderView(generics.RetrieveAPIView):
    ...
    def get(self, request, file_id):
        ...
        # Prepare chapters - check for translations first
        sanitized_chapters = []
        ...
        if target_lang != 'auto':  # Only look for translations if target language is specified
            # Look for specific chapter translations first
            if chapter_param is not None:
                try:
                    chapter_index = int(chapter_param)
                    log.info(f"[EpubReader] Buscando tradução para capítulo específico: {chapter_index}")
                    
                    translation = TranslatedEpub.objects.filter(
                    ...
                    if translation and translation.translated_chapters:
                        log.info(f"[EpubReader] Tradução encontrada para capítulo {chapter_index}")
                        chapter_translations[chapter_index] = translation.translated_chapters[0]
                    else:
                        log.info(f"[EpubReader] Nenhuma tradução encontrada para capítulo {chapter_index}")
                except ValueError:
                    pass
            
            # Look for full book translation as fallback
            if not chapter_translations:
                ...
                if full_translation and full_translation.translated_chapters:
                    log.info(f"[EpubReader] Tradução completa encontrada com {len(full_translation.translated_chapters)} capítulos")
                    for i, translated_chapter in enumerate(full_translation.translated_chapters):
                        chapter_translations[i] = translated_chapter
                else:
                    log.info(f"[EpubReader] Nenhuma tradução completa encontrada")
        
        # Build chapters with translation when available
        for i, ch in enumerate(chapters_obj):
            if isinstance(ch, dict):
                # Check if we have a translation for this chapter
                if i in chapter_translations:
                    log.info(f"[EpubReader] Usando tradução para capítulo {i}")
                    translated_ch = chapter_translations[i]
                    raw_content = translated_ch.get('content') or ch.get('content') or ''
                    title = translated_ch.get('title') or ch.get('title') or ''
                else:
                    log.info(f"[EpubReader] Usando conteúdo original para capítulo {i}")
                ...
                sanitized_chapters.append({
                    ...
                    'translated': i in chapter_translations  # Flag to indicate if this chapter is translated
                })
...
class AO3ImportView(generics.GenericAPIView):
    ...
    def _post_impl(self, request, *args, **kwargs):
        ...
        # First, clean up any orphaned ExtractedEpub records (safety check)
        orphaned_extracted = ExtractedEpub.objects.filter(uploaded_file__isnull=True)
        if orphaned_extracted.exists():
            log.warning(f"[AO3Import][{corr_id}] Removendo {orphaned_extracted.count()} registros órfãos de ExtractedEpub")
            orphaned_extracted.delete()
        
        # Now check for duplicates with a more robust query
        existing_extracted = ExtractedEpub.objects.select_related('uploaded_file').filter(
            metadata__external_work_id=work_id, 
            uploaded_file__user=request.user,
            uploaded_file__isnull=False  # Ensure uploaded_file exists
        ).first()
...
def cleanup_orphaned_records():
    ...
    with transaction.atomic():
        # Encontrar ExtractedEpub órfãos (sem UploadedFile)
        orphaned_extracted = ExtractedEpub.objects.filter(uploaded_file__isnull=True)
        ...
        # Encontrar TranslatedEpub órfãos (sem ExtractedEpub)
        ...
        # Encontrar ReadingProgress órfãos (sem ExtractedEpub)
...
class DiagnosticsView(generics.GenericAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        # Verificar registros órfãos globalmente (apenas para admin)
        orphaned_data = {}
        if user.is_staff:
            orphaned_extracted = ExtractedEpub.objects.filter(uploaded_file__isnull=True).count()
            orphaned_translated = TranslatedEpub.objects.filter(extracted_epub__isnull=True).count()
            orphaned_progress = ReadingProgress.objects.filter(extracted_epub__isnull=True).count()
            
            orphaned_data = {
                'orphaned_extracted_epubs': orphaned_extracted,
                'orphaned_translated_epubs': orphaned_translated,
                'orphaned_reading_progress': orphaned_progress
            }
        ...
        return Response({
            ...
            'orphaned_data': orphaned_data if user.is_staff else 'Only available for staff users'
        })
    
    def post(self, request, *args, **kwargs):
        """
        Executa limpeza de registros órfãos (apenas para admin)
        """
        if not request.user.is_staff:
            return Response({'error': 'Permission denied'}, status=status.HTTP_403_FORBIDDEN)
        
        cleaned_data = cleanup_orphaned_records()
        return Response({
